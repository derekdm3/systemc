...
#define RING_DEBUG(fmt, ...) printf ("%s:%s " fmt, __func__ , qemu_get_queue(etsec->nic).name, ## __VA_ARGS__)
#else
#define RING_DEBUG(...)
#endif  /* ETSEC_RING_DEBUG */

#define RING_DEBUG_A(fmt, ...) printf ("%s:%s " fmt, __func__ , qemu_get_queue(etsec->nic).name, ## __VA_ARGS__)
            ...
            if (remaining_data == 0) {
                ...
                if (size >= etsec->regs[MAXFRM].value) {
                    ...
                    printf("%s frame length violation: size:%d MAXFRM:%d\n",
                           __func__, (unsigned int)size, etsec->regs[MAXFRM].value);
